...
class FailFrame(QFrame):
    ...
    # fail image (mask)
    __image_mask_step = 0
    __image_mask_left_start_x, __image_mask_left_start_y = 68, 68
    __image_mask_left_end_x, __image_mask_left_end_y = __image_mask_left_start_x, __image_mask_left_start_y
    __image_mask_right_start_x, __image_mask_right_start_y = 102, 68
    __image_mask_right_end_x, __image_mask_right_end_y = __image_mask_right_start_x, __image_mask_right_start_y
    ...
    def __draw_image_animation(self):
        ...
        if self.__image_circle_animation_completed:
            if self.__image_mask_step <= 16:
                self.__image_mask_left_end_x += 2
                self.__image_mask_left_end_y += 2
                painter.drawLine(self.__image_mask_left_start_x, self.__image_mask_left_start_y,
                                 self.__image_mask_left_end_x, self.__image_mask_left_end_y)

            elif self.__image_mask_step <= 33:
                self.__image_mask_right_end_x -= 2
                self.__image_mask_right_end_y += 2
                painter.drawLine(self.__image_mask_left_start_x, self.__image_mask_left_start_y,
                                 self.__image_mask_left_end_x, self.__image_mask_left_end_y)
                painter.drawLine(self.__image_mask_right_start_x, self.__image_mask_right_start_y,
                                 self.__image_mask_right_end_x, self.__image_mask_right_end_y)
...
class CircleMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.fail_frame = FailFrame(self)
        self.fail_frame.move(center_x, center_y)
